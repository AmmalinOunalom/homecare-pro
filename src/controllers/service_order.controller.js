...
const send_sms_to_employee = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const employee = yield employees_model_1.employees_model.get_employee_by_phone(employee_phone);
        if (!employee) {
            res.status(404).json({ error: 'Employee phone number not found' });
            return;
        }
...
ລາຍລະອຽດ: ${details}
ແຜນທີ່: ${mapLink}`;
        const sid = yield (0, sms_utils_1.sendSMS)(employee_phone, message);
